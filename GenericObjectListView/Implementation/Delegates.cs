...
namespace BrightIdeasSoftware {
    ...
    /// </summary>
    public delegate void AspectPutterDelegate<T>(T rowObject, T newValue);

    /// <summary>
    /// These delegates can be used to convert an aspect value to a display string,
    /// instead of using the default ToString()
    /// </summary>
    public delegate string AspectToStringConverterDelegate<T>(T value);

    /// <summary>
    /// These delegates are used to get the tooltip for a cell
    /// </summary>
    public delegate String CellToolTipGetterDelegate<T>(OLVColumn<T> column, T modelObject);
    ...
    /// </remarks>
    public delegate CheckState CheckStateGetterDelegate<T>(T rowObject);
    ...
    /// <returns></returns>
    public delegate bool BooleanCheckStateGetterDelegate<T>(T rowObject);

    /// <summary>
    /// These delegates are used to put a changed check state back into a model object
    /// </summary>
    public delegate CheckState CheckStatePutterDelegate<T>(T rowObject, CheckState newValue);
    ...
    /// <returns></returns>
    public delegate bool BooleanCheckStatePutterDelegate<T>(T rowObject, bool newValue);

    /// <summary>
    /// These delegates are used to get the renderer for a particular cell
    /// </summary>
    public delegate IRenderer<T> CellRendererGetterDelegate<T>(T rowObject, OLVColumn<T> column);
    ...
    /// </summary>
    public delegate bool HeaderDrawingDelegate<T>(Graphics g, Rectangle r, int columnIndex, OLVColumn<T> column, bool isPressed, HeaderStateStyle stateStyle);

    /// <summary>
    /// These delegates are used to retrieve the object that is the key of the group to which the given row belongs.
    /// </summary>
    public delegate Object GroupKeyGetterDelegate<T>(T rowObject);
    ...
    /// </summary>
    public delegate String HeaderToolTipGetterDelegate<T>(OLVColumn<T> column);
        
    /// <summary>
    /// These delegates are used to fetch the image selector that should be used
    /// to choose an image for this column.
    /// </summary>
    public delegate Object ImageGetterDelegate<T>(T rowObject);

    /// <summary>
    /// These delegates are used to draw a cell
    /// </summary>
    public delegate bool RenderDelegate<T>(EventArgs e, Graphics g, Rectangle r, T rowObject);

    /// <summary>
    /// These delegates are used to fetch a row object for virtual lists
    /// </summary>
    public delegate T RowGetterDelegate<T>(int rowIndex);

    /// <summary>
    /// These delegates are used to format a listviewitem before it is added to the control.
    /// </summary>
    public delegate void RowFormatterDelegate<T>(OLVListItem<T> olvItem);
    ...
    /// </summary>
    public delegate void SortDelegate<T>(OLVColumn<T> column, SortOrder sortOrder);
