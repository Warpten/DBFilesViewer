...
namespace BrightIdeasSoftware
{
    ...
    public class Munger<T>
    {
        ...
        public bool PutProperty(T target, string propertyName, object value) {
            try {
                var munger = new Munger<T>(propertyName);
        ...
        /// </remarks>
        public static bool IgnoreMissingAspects { get; set; } = true;
        ...
        /// <param name="target">The object that will be peeked</param>
        /// <returns>The value read from the target</returns>
        public object GetValueEx(T target) {
            if (Parts.Count == 0)
                return null;
        ...
        public bool PutValue(T target, object value)
        {
            if (Parts.Count == 0)
                return false;
            ...
            if (Parts.Count > 1) {
                ...
                try {
                    currentTarget = EvaluateParts(currentTarget, parts);
                } catch (MungerException ex) {
            ...
            try {
                return lastPart.PutValue(currentTarget, value);
            } catch (MungerException ex) {
                ReportPutValueException(ex);
            }
