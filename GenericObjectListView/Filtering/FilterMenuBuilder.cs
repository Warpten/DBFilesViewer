...
namespace BrightIdeasSoftware {
    ...
    /// </remarks>
    public class FilterMenuBuilder<T>
    {
        ...
        public virtual ToolStripDropDown MakeFilterMenu(ToolStripDropDown strip, ObjectListView<T> listView, OLVColumn<T> column) {
            if (strip == null)
        ...
        /// <param name="column"></param>
        /// <returns></returns>
        protected virtual List<ICluster<T>> Cluster(IClusteringStrategy<T> strategy, ObjectListView<T> listView, OLVColumn<T> column) {
            // Build a map that correlates cluster key to clusters
            var map = new NullableDictionary<object, ICluster<T>>();
            var count = 0;
            ...
            return new List<ICluster<T>>(map.Values);
        }
        ...
        protected virtual void SortClusters(IClusteringStrategy<T> strategy, List<ICluster<T>> clusters) =>
            clusters.Sort();
        ...
        /// <param name="clusters"></param>
        /// <returns></returns>
        protected virtual ToolStripMenuItem CreateFilteringMenuItem(OLVColumn<T> column, List<ICluster<T>> clusters) {
            var checkedList = new ToolStripCheckedListBox { Tag = column };
            ...
            var clearAll = new ToolStripMenuItem(CLEAR_ALL_FILTERS_LABEL, ClearFilteringImage, (sender, args) => ClearAllFilters(column));
            var apply = new ToolStripMenuItem(APPLY_LABEL, FilteringImage, (sender, args) => EnactFilter(checkedList, column));
            var subMenu = new ToolStripMenuItem(FILTERING_LABEL, null, clearAll, new ToolStripSeparator(), checkedList, apply);
            return subMenu;
        }
        ...
        protected virtual void HandleItemChecked(object sender, ItemCheckEventArgs e) {
            ...
            if (checkedList == null)
                return;
            var column = checkedList.Tag as OLVColumn<T>;
            if (column == null)
                return;
            var listView = column.ListView as ObjectListView<T>;
        ...
        protected virtual void ClearAllFilters(OLVColumn<T> column) {

            var olv = column.ListView as ObjectListView<T>;
        ...
        protected virtual void EnactFilter(ToolStripCheckedListBox checkedList, OLVColumn<T> column) {
