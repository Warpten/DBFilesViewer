...
namespace DBFilesClient.NET.WDB5
{
    internal sealed class Reader<T> : Reader where T : class, new()
    {
        ...
        internal override void Load()
        {
            ...
            var minIndex = ReadUInt32();
            var maxIndex = ReadUInt32();
            ...
            if (FileHeader.HasIndexTable)
            {
                ...
                idTable = new uint[RecordCount];
                for (var i = 0; i < RecordCount; ++i)
                    idTable[i] = ReadUInt32();
            }

            var offsetMap = new Dictionary<uint /* recordIndex */, long /* absoluteOffset */>();
            ...
            else
            {
                ...
                for (var i = 0u; i < maxIndex - minIndex + 1; ++i)
                {
            ...
            {
                var newIndex = ReadUInt32();
                var oldIndex = ReadUInt32();
        ...
        private void LoadRecord(long recordPosition, uint key, bool forceKey = false)
        {
            ...
            else
            {
                ...
                {
                    case 4:
                        TriggerRecordLoaded(ReadUInt32(), record);
                        break;
                    case 3:
                        TriggerRecordLoaded(ReadUInt24(), record);
                        break;
                    case 2:
                        TriggerRecordLoaded(ReadUInt16(), record);
                        break;
                    case 1:
                        TriggerRecordLoaded(ReadByte(), record);
                        break;
                }
            }
        }
            ...
            foreach (var fieldInfo in fields)
            {
                ...
                if (typeCode == TypeCode.Object)
                {
                    ...
                    if (baseType?.GetGenericTypeDefinition() != typeof(IObjectType<>))
                        throw new InvalidStructureException("Only objects inheriting IObjectType<T> can be loaded.");
