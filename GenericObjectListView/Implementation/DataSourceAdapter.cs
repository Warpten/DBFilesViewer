...
namespace BrightIdeasSoftware
{
    ...
    /// </summary>
    public class DataSourceAdapter<T> : IDisposable
    {
        ...
        /// </summary>
        public DataSourceAdapter(ObjectListView<T> olv) {
        ...
        protected virtual void BindListView(ObjectListView<T> olv) {
            if (olv == null)
                return;

            olv.Freezing += HandleListViewFreezing;
            olv.SelectionChanged += HandleListViewSelectionChanged;
        ...
        /// </summary>
        /// <param name="olv"></param>
        protected virtual void UnbindListView(ObjectListView<T> olv) {
            if (olv == null)
                return;
        ...
        protected virtual void SetListContents() {
            ListView.Objects = CurrencyManager.List as IList<T>;
        }
        ...
        protected virtual void CreateColumnsFromSource() {
            ...
            var generator = Generator<T>.Instance as Generator<T> ?? new Generator<T>();
        ...
        protected virtual bool ShouldCreateColumn(PropertyDescriptor property) {
            ...
            if (ListView.AllColumns.Cast<OLVColumn<T>>().Any(x => x.Name == property.Name))
                return false;
        ...
        /// <param name="property"></param>
        protected virtual void ConfigureColumn(OLVColumn<T> column, PropertyDescriptor property) {
            ...
            if (property.PropertyType == typeof(byte[]))
                column.Renderer = new ImageRenderer<T>();
        }
        ...
        protected virtual void CreateMissingAspectGettersAndPutters() {
            foreach (OLVColumn<T> x in ListView.AllColumns) {
                var column = x; // stack based variable accessible from closures
                if (column.AspectGetter == null) {
                    column.AspectGetter = delegate(T row) {
                        // In most cases, rows will be DataRowView objects
                        var drv = row as DataRowView;
                        if (drv == null)
                            return drv[column.Name];
                        return (drv.Row.RowState == DataRowState.Detached) ? null : drv[column.Name];
                    };
                }
            }
        }
        ...
        protected virtual void CurrencyManagerListChanged(object sender, ListChangedEventArgs e) {
            ...
            sw.Stop();
            Debug.WriteLine("PERF - Processing {0} event on {1} rows took {2}ms", e.ListChangedType, ListView.GetItemCount(), sw.ElapsedMilliseconds);
        ...
        protected virtual void HandleListChangedItemChanged(ListChangedEventArgs e) {
            ...
            ListView.RefreshObject((T)changedRow);
        }
        ...
        protected virtual void ChangePosition(int index) {
            ...
            ListView.SelectedObject = (T)CurrencyManager.List[index];
        ...
        #region ObjectListView<T> event handlers
