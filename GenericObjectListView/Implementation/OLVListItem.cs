...
namespace BrightIdeasSoftware {
    ...
    /// </summary>
    public class OLVListItem<T> : ListViewItem {
        #region Constructors

        /// <summary>
        /// Create a OLVListItem for the given row object
        /// </summary>
        public OLVListItem(T rowObject) {
            RowObject = rowObject;
        }
        ...
        /// </summary>
        public OLVListItem(T rowObject, string text, object image)
            : base(text, -1) {
            RowObject = rowObject;
            imageSelector = image;
        }
        ...
        new public bool Checked {
            ...
            set {
                if (Checked != value) {
                    if (value)
                        ((ObjectListView<T>)ListView).CheckObject(RowObject);
                    else
                        ((ObjectListView<T>)ListView).UncheckObject(RowObject);
                }
            }
        }
        ...
        public IDecoration<T> Decoration {
            get
            {
                if (HasDecoration)
        ...
        /// </summary>
        public IList<IDecoration<T>> Decorations {
            get {
                if (decorations == null)
                    decorations = new List<IDecoration<T>>();
                return decorations;
            }
        }
        private IList<IDecoration<T>> decorations;
        ...
        public bool HasAnyHyperlinks {
            get {
                foreach (OLVListSubItem<T> subItem in SubItems) {
        ...
        public virtual OLVListSubItem<T> GetSubItem(int index) {
            if (index >= 0 && index < SubItems.Count)
                return (OLVListSubItem<T>)SubItems[index];
