...
namespace BrightIdeasSoftware {
    ...
    /// </summary>
    public class OLVDataObject<T> : DataObject {
        ...
        public OLVDataObject(ObjectListView<T> olv)
            : this(olv, olv.SelectedObjects as List<T>) {
        }
        ...
        public OLVDataObject(ObjectListView<T> olv, IList<T> modelObjects) {
            ListView = olv;
        ...
        /// </summary>
        public IList<T> ModelObjects { get; }
        ...
        public void CreateTextFormats() {
            ...
            SetData(exporter.ExportTo(OLVExporter<T>.ExportFormat.TabSeparated));
            var exportTo = exporter.ExportTo(OLVExporter<T>.ExportFormat.CSV);
            if (!string.IsNullOrEmpty(exportTo))
                SetText(exportTo, TextDataFormat.CommaSeparatedValue);
            SetText(ConvertToHtmlFragment(exporter.ExportTo(OLVExporter<T>.ExportFormat.HTML)), TextDataFormat.Html);
        }
        ...
        /// </summary>
        /// <returns></returns>
        protected OLVExporter<T> CreateExporter() {
            var exporter = new OLVExporter<T>(ListView)
            {
