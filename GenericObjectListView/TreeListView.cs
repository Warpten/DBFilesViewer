/*
 * TreeListView<T> - A listview that can show a tree of objects in a column
 *
...
namespace BrightIdeasSoftware
{
    ...
    ///     </para>
    /// </remarks>
    public partial class TreeListView<T> : VirtualObjectListView<T>
    {
        /// <summary>
...
// ReSharper restore DoNotCallOverridableMethodsInConstructor
        ...
        [Browsable(false),
         DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public IEnumerable<T> ExpandedObjects
        {
        ...
        [Browsable(false),
         DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public override IListFilter<T> ListFilter
        {
            get { return null; }
            set { Debug.Assert(value == null, "TreeListView<T> do not support ListFilters"); }
        }
        ...
        [Browsable(false)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public override IEnumerable<T> ObjectsForClustering
        {
            get
            {
        ...
        /// </summary>
        /// <remarks>
        ///     Setting this does <b>NOT</b> reset the state of the control.
        ///     In particular, it does not collapse branches.
        /// </remarks>
        [Browsable(false),
         DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public virtual IEnumerable<T> Roots
        {
            get { return TreeModel.RootObjects; }
            set
            {
                TreeColumnRenderer = TreeColumnRenderer;
                TreeModel.RootObjects = value ?? new List<T>();
        ...
        [Category("ObjectListView"),
         ...
         DefaultValue(true)]
        public virtual bool UseWaitCursorWhenExpanding { get; set; } = true;
        ...
        /// <returns></returns>
        public virtual bool IsExpanded(T model)
        {
        ...
        /// </summary>
        /// <param name="model"></param>
        public virtual void Collapse(T model)
        {
            if (GetItemCount() == 0)
                return;

            var item = ModelToItem(model);
            var args = new TreeBranchCollapsingEventArgs<T>(model, item);
            OnCollapsing(args);
            if (args.Canceled)
                return;
            ...
            if (index >= 0)
            {
                ...
                OnCollapsed(new TreeBranchCollapsedEventArgs<T>(model, item));
            }
        }
        ...
        public virtual void CollapseAll()
        {
            ...
            var args = new TreeBranchCollapsingEventArgs<T>(default(T), null);
            OnCollapsing(args);
            if (args.Canceled)
                return;
            ...
            if (index >= 0)
            {
                ...
                if (index < GetItemCount())
                    RedrawItems(index, GetItemCount() - 1, true);
                OnCollapsed(new TreeBranchCollapsedEventArgs<T>(default(T), null));
            }
        }
        ...
        /// </summary>
        /// <param name="model"></param>
        public virtual void Expand(T model)
        {
            if (GetItemCount() == 0)
                return;
            ...
            var item = ModelToItem(model);
            var args = new TreeBranchExpandingEventArgs<T>(model, item);
            OnExpanding(args);
            if (args.Canceled)
                return;
            ...
            OnExpanded(new TreeBranchExpandedEventArgs<T>(model, item));
        ...
        public virtual void ExpandAll()
        {
            ...
            // Give the world a chance to cancel the expansion
            var args = new TreeBranchExpandingEventArgs<T>(default(T), null);
            OnExpanding(args);
            if (args.Canceled)
                return;
            ...
            using (SuspendSelectionEventsDuring())
                SelectedObjects = selection;
            RedrawItems(index, GetItemCount() - 1, true);
            OnExpanded(new TreeBranchExpandedEventArgs<T>(default(T), null));
        }
        ...
        protected virtual void RebuildAll(IList<T> selected, IEnumerable<T> expanded, IList<T> checkedObjects)
        {
        ...
        public virtual void Reveal(T modelToReveal, bool selectAfterReveal)
        {
            // Collect all the ancestors of the model
            var ancestors = GetAncestors(modelToReveal).ToList();
        ...
        public override void RefreshObjects(IList<T> modelObjects)
        {
            ...
            foreach (T model in modelsAndParents.Keys)
            {
                if (model == null)
                    continue;
        ...
        protected override bool SetObjectCheckedness(T modelObject, CheckState state)
        {
            ...
            var args = new List<T> {modelObject};
        ...
        private IEnumerable<T> GetChildrenWithoutExpanding(T model)
        {
            var br = TreeModel.GetBranch(model);
        ...
        /// </summary>
        /// <param name="model"></param>
        public virtual void ToggleExpansion(T model)
        {
            if (IsExpanded(model))
                Collapse(model);
            else
        ...
        /// <returns></returns>
        /// <remarks>The given model must have already been seen in the tree.</remarks>
        public virtual T GetParent(T model)
        {
            var br = TreeModel.GetBranch(model);
            return br == null || br.ParentBranch == null ? default(T) : br.ParentBranch.Model;
        }
        ...
        ///     </para>
        /// </remarks>
        public virtual IEnumerable<T> GetChildren(T model)
        {
            var br = TreeModel.GetBranch(model);
            if (br == null || !br.CanExpand)
                return new List<T>();
        ...
        public delegate bool CanExpandGetterDelegate(T model);
        ...
        public delegate IEnumerable<T> ChildrenGetterDelegate(T model);
        ...
        public delegate T ParentGetterDelegate(T model);
        ...
        public delegate Tree TreeFactoryDelegate(TreeListView<T> view);
        ...
        /// <returns></returns>
        protected override bool ProcessLButtonDown(OlvListViewHitTestInfo<T> hti)
        {
        ...
        public override OLVListItem<T> MakeListViewItem(int itemIndex)
        {
        ...
        protected virtual void RecalculateHierarchicalCheckBoxGraph(IList<T> toCheck)
        {
        ...
        /// <remarks>This only makes sense when HierarchicalCheckboxes is true.</remarks>
        /// <param name="modelObject"></param>
        protected virtual void RecalculateSingleHierarchicalCheckBox(T modelObject)
        {
            if (modelObject == null)
                return;
        ...
        private bool CanExpandUncached(T model)
        {
            return CanExpandGetter != null && model != null && CanExpandGetter(model);
        }

        private IEnumerable<T> GetChildrenUncached(T model)
        {
            return ChildrenGetter != null && model != null ? ChildrenGetter(model) : new List<T>();
        }
        ...
        protected virtual IEnumerable<T> CalculateDistinctAncestors(IList<T> toCheck)
        {
            if (toCheck.Count == 1)
            {
            ...
            else
            {
                ...
                var uniqueAncestors = new List<T>();
                var alreadySeen = new Dictionary<T, bool>();
        ...
        protected virtual IEnumerable<T> GetAncestors(T model)
        {
        ...
        {
            var focused = FocusedItem as OLVListItem<T>;
            if (focused == null)
            {
                base.OnKeyDown(e);
                return;
        ...
        /// </remarks>
        public class Tree : IVirtualListDataSource<T>, IFilterableDataSource<T>
        {
            /// <summary>
            ///     Create a Tree
            /// </summary>
            /// <param name="treeView"></param>
            public Tree(TreeListView<T> treeView)
            {
                TreeView = treeView;
                trunk = new Branch(null, this, default(T));
            ...
            /// </summary>
            public IEnumerable<T> RootObjects
            {
            ...
            /// </summary>
            public TreeListView<T> TreeView { get; }
            ...
            public virtual int Collapse(T model)
            {
                ...
                ((List<T>) objectList).RemoveRange(index + 1, count);
                RebuildObjectMap(0);
                return index;
            }
            ...
            /// </remarks>
            public virtual int Expand(T model)
            {
                var br = GetBranch(model);
            ...
            ///     isn't in the tree.
            /// </returns>
            public virtual Branch GetBranch(T model)
            {
                if (model == null)
                    return null;
            ...
            public virtual int GetVisibleDescendentCount(T model)
            {
                var br = GetBranch(model);
            ...
            /// <param name="model"></param>
            /// <returns>The index of the model in flat list version of the tree</returns>
            public virtual int RebuildChildren(T model)
            {
                var br = GetBranch(model);
            ...
            /// <param name="model"></param>
            /// <returns></returns>
            internal bool IsModelExpanded(T model)
            {
            ...
            internal void SetModelExpanded(T model, bool isExpanded)
            {
            ...
            protected virtual void InsertChildren(Branch br, int index)
            {
                ...
                ((List<T>) objectList).InsertRange(index, br.Flatten());
                RebuildObjectMap(index);
            }
            ...
            /// <param name="model"></param>
            /// <returns></returns>
            internal Branch MakeBranch(Branch parent, T model)
            {
            ...
            /// <param name="n"></param>
            /// <returns></returns>
            public virtual T GetNthObject(int n)
            {
                return objectList[n];
            }
            ...
            /// <param name="model"></param>
            /// <returns></returns>
            public virtual int GetObjectIndex(T model)
            {
                int index;
            ...
            /// <returns></returns>
            public virtual int SearchText(string value, int first, int last, OLVColumn<T> column)
            {
                return AbstractVirtualListDataSource<T>.DefaultSearchText(value, first, last, column, this);
            }
            ...
            public virtual void Sort(OLVColumn<T> column, SortOrder order)
            {
            ...
            protected virtual BranchComparer GetBranchComparer()
            {
                ...
                return new BranchComparer(new ModelObjectComparer<T>(
                    lastSortColumn,
                    lastSortOrder,
            ...
            {
                var newRoots = TreeView.Roots.ToList();
                newRoots.AddRange(modelObjects);
                SetObjects(newRoots);
            }
            ...
            /// <param name="modelObjects"></param>
            public void InsertObjects(int index, ICollection<T> modelObjects)
            {
            ...
            public virtual void SetObjects(IEnumerable<T> collection)
            {
            ...
            /// <param name="index"></param>
            /// <param name="modelObject"></param>
            public void UpdateObject(int index, T modelObject)
            {
                var newRoots = TreeView.Roots.ToList();
            ...
            public void ApplyFilters(IModelFilter<T> mFilter, IListFilter<T> lFilter)
            {
                modelFilter = mFilter;
                listFilter = lFilter;
                RebuildList();
            ...
            internal bool IncludeModel(T model)
            {
            ...
            private OLVColumn<T> lastSortColumn;
            private SortOrder lastSortOrder;
            private readonly Dictionary<T, Branch> mapObjectToBranch = new Dictionary<T, Branch>();
// ReSharper disable once InconsistentNaming
            internal Dictionary<T, bool> mapObjectToExpanded = new Dictionary<T, bool>();
            private readonly Dictionary<T, int> mapObjectToIndex = new Dictionary<T, int>();
            private IList<T> objectList = new List<T>();
            private readonly Branch trunk;

            /// <summary>
            /// </summary>
// ReSharper disable once InconsistentNaming
            protected IModelFilter<T> modelFilter;

            /// <summary>
            /// </summary>
// ReSharper disable once InconsistentNaming
            protected IListFilter<T> listFilter;
        }
        ...
        public class Branch
        {
            ...
            /// <param name="model"></param>
            public Branch(Branch parent, Tree tree, T model)
            {
                ParentBranch = parent;
                Tree = tree;
                Model = model;
            ...
            private void AddChild(T childModel)
            {
            ...
            /// </summary>
            /// <returns></returns>
            public virtual IList<T> Flatten()
            {
                IList<T> flatList = new List<T>();
                if (IsExpanded)
            ...
            public virtual void FlattenOnto(IList<T> flatList)
            {
        ...
        public class BranchComparer : IComparer<Branch>
        {
            ...
            public BranchComparer(IComparer<T> actualComparer)
            {
