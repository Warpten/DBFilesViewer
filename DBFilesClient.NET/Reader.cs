...
namespace DBFilesClient.NET
{
    public abstract class Reader : BinaryReader
    {
        ...
        public string ReadInlineString()
        {
            ...
            BaseStream.Position -= stringLength + 1; // \0 was read as well
            return Encoding.UTF8.GetString(ReadBytes(stringLength + 1), 0, stringLength); // Read again, but skip null terminator.
        }
        ...
        public event Action<uint, object> OnRecordLoaded;

        protected void TriggerRecordLoaded(uint key, object record) => OnRecordLoaded?.Invoke(key, record);
