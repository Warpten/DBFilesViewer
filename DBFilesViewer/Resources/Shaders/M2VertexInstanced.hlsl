...
float2 getEnv(VertexInput input)
{
    ...
    float3 vertex = mul(input.position, worldViewMat);
    float3 normal = mul(input.normal, worldViewMat);
    float3 normPos = -normalize(vertex.xyz);
    float3 tmp = (normPos - (normal * (2.0f * dot(normPos, normal))));
    tmp = float3(tmp.x, tmp.y, tmp.z + 1.0f);
    return (normalize(tmp).xy * 0.5f) + float2(0.5f, 0.5f);
}
